...
impl Display for EIP55Address {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "0x{}", encode_rskip60(self.0.as_bytes(), RSKIPChain::Ethereum))
    }
}
...
mod tests {
    ...
    async fn test_rskip60_eth() {
        assert_eq!(
            encode_rskip60(hex::decode("2b5c7025998f88550ef2fece8bf87935f542c190").unwrap().as_slice(), RSKIPChain::Ethereum),
            "2B5c7025998f88550Ef2fEce8bf87935f542C190".to_string()
        );

        assert_eq!(
            encode_rskip60(hex::decode("77c0f878fde3d1ad2830ca9e46d7c47c951a93fb").unwrap().as_slice(), RSKIPChain::Ethereum),
    ...
    async fn test_rskip60_rootstock() {
        assert_eq!(
            encode_rskip60(hex::decode("5aaeb6053f3e94c9b9a09f33669435e7ef1beaed").unwrap().as_slice(), RSKIPChain::Other(30)),
            "5aaEB6053f3e94c9b9a09f33669435E7ef1bEAeD".to_string()
        );

        assert_eq!(
            encode_rskip60(hex::decode("fb6916095ca1df60bb79ce92ce3ea74c37c5d359").unwrap().as_slice(), RSKIPChain::Other(30)),
            "Fb6916095cA1Df60bb79ce92cE3EA74c37c5d359".to_string()
        );
    }

    #[tokio::test]
    async fn test_eip55address_display() {
        assert_eq!(
            EIP55Address(Address::from_str("0x2b5c7025998f88550ef2fece8bf87935f542c190").unwrap()).to_string(),
